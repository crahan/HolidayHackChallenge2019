...
![Lock 6](../img/challenges/c11/c11_6.png)
...
Unlike the previous 9 locks this one can't be solved with one or two clicks. Start by removing the `cover` class from the div which will uncover, pun intended, the inside of the lock. In the bottom right hand corner, vertically, and in small print is the code `KD29XJ37`.

![Lock 10 Inside](../img/challenges/c11/c11_10.png)

Try entering the code and be greeted with an error in the JavaScript console, stating 'Error: Missing Macaroni!'. The problem is there's three different classes, `macaroni` being one of them, missing on the div that previously had the `cover` class enabled.

![Obfuscated JavaScript](../img/challenges/c11/c11_11.png)

We will need to deobfuscate the JavaScript code to better understand what exactly it's trying to check inside of that `try/catch` block. One option is to use the developer tools JavaScript console to evaluate small pieces of the code. For example, entering `_0x3170('0x56')` in the console will print "Missing macaroni!". Rinse and repeat until the code becomes more readable.
...
It appears that the three component, defined as CSS classes, we're missing on this lock are `macaroni`, `swab`, and `gnome`. You will find 3 divs  containing each of the required classes spread around the DOM. Fix things by dragging all of them inside of the lock div and you'll be able to open this final lock using the code `KD29XJ37`.

![Lock 10](../img/challenges/c11/c11_13.png)
![Note](../img/challenges/c11/c11_14.png)

Say whaaaaaa?! The Tooth Fairy is behind all of this madness?
